...
"""check-yeps: Check YEPs for common mistakes.

Usage: check-yeps [-d | --detailed] <YEP files...>

Only the YEPs specified are checked.
If none are specified, all YEPs are checked.
...
# get the directory with the YEP sources
ROOT_DIR = Path(__file__).resolve().parent
YEP_ROOT = ROOT_DIR / "yeps"

# See YEP 12 for the order
...
REQUIRED_HEADERS = frozenset({"YEP", "Title", "Author", "Status", "Type", "Created"})

# See YEP 1 for the full list
...
# YEPs that are allowed to link directly to YEPs
SKIP_DIRECT_YEP_LINK_CHECK = frozenset({"0009", "0287", "0676", "0684", "8001"})
...
def check(filenames: Sequence[str] = (), /) -> int:
    ...
    if filenames:
        ...
        filenames = YEP_ROOT.glob("yep-????.rst")
    if (count := sum(map(check_file, filenames))) > 0:
        s = "s" * (count != 1)
        print(f"check-yeps failed: {count} error{s}", file=sys.stderr)
        return 1
    return 0
...
def check_file(filename: Path, /) -> int:
    ...
    try:
        ...
        return _output_error(filename, [""], [(0, "Could not read YEP!")])
    else:
        lines = content.splitlines()
        return _output_error(filename, lines, check_yeps(filename, lines))


def check_yeps(filename: Path, lines: Sequence[str], /) -> MessageIterator:
    yield from check_headers(lines)
    for line_num, line in enumerate(lines, start=1):
        if filename.stem.removeprefix("yep-") in SKIP_DIRECT_YEP_LINK_CHECK:
            continue
        yield from check_direct_links(line_num, line.lstrip())


def check_headers(lines: Sequence[str], /) -> MessageIterator:
    yield from _validate_yep_number(next(iter(lines), ""))
...
def _validate_header(header: str, line_num: int, content: str) -> MessageIterator:
    if header == "Title":
    ...
    elif header in {"BDFL-Delegate", "YEP-Delegate"}:
        ...
        yield from _validate_yep_references(line_num, content)
    elif header == "Created":
        yield from _validate_created(line_num, content)
    elif header == "Yocto-Version":
        yield from _validate_yocto_version(line_num, content)
...
def check_direct_links(line_num: int, line: str) -> MessageIterator:
    """Check that YEPs and RFCs aren't linked directly"""

    line = line.lower()
    if "dev/yeps/yep-" in line or "JPEWdev.github.io/yeps/yep-" in line:
        yield line_num, "Use the :yep:`NNN` role to refer to YEPs"
...
###########################
#  YEP Header Validators  #
###########################


def _validate_required_headers(found_headers: KeysView[str]) -> MessageIterator:
    """YEPs must have all required headers, in the YEP 12 order"""
    ...
    if list(found_headers) != ordered_headers:
        ...
        yield 1, "Headers must be in YEP 12 order. Correct order: " + order_str


def _validate_yep_number(line: str) -> MessageIterator:
    """'YEP' header must be a number 1-9999"""

    if not line.startswith("YEP: "):
        yield 1, "YEP must begin with the 'YEP:' header"
        return

    yep_number = line.removeprefix("YEP: ").lstrip()
    yield from _yep_num(1, yep_number, "'YEP:' header")
...
def _validate_title(line_num: int, line: str) -> MessageIterator:
    ...
    if len(line) == 0:
        yield line_num, "YEP must have a title"
    elif len(line) > 79:
        yield line_num, "YEP title must be less than 80 characters"
...
def _validate_discussions_to(line_num: int, line: str) -> MessageIterator:
    ...
    if line.startswith("https://"):
        return
    for suffix in "@lists.openembedded.org", "@lists.yoctoproject.org":
...
def _validate_status(line_num: int, line: str) -> MessageIterator:
    """'Status' must be a valid YEP status"""

    if line not in ALL_STATUSES:
        yield line_num, "Status must be a valid YEP status"


def _validate_type(line_num: int, line: str) -> MessageIterator:
    """'Type' must be a valid YEP type"""

    if line not in {"Standards Track", "Informational", "Process"}:
        yield line_num, "Type must be a valid YEP type"
...
def _validate_yep_references(line_num: int, line: str) -> MessageIterator:
    """`Requires`/`Replaces`/`Superseded-By` must be 'NNN' YEP IDs"""

    line = line.removesuffix(",").rstrip()
    if line.count(", ") != line.count(","):
        yield line_num, "YEP references must be separated by comma-spaces (', ')"
        return

    references = line.split(", ")
    for reference in references:
        yield from _yep_num(line_num, reference, "YEP reference")
...
def _validate_yocto_version(line_num: int, line: str) -> MessageIterator:
    """'Yocto-Version' must be an ``X.Y`` version"""

    versions = line.split(", ")
    for version in versions:
        if version.count(".") != 1:
            yield line_num, f"Yocto-Version must have two segments: {version}"
            continue

        major, minor = version.split(".", 1)

        if not _is_digits(minor):
            yield line_num, f"Yocto-Version minor part must be numeric: {version}"
        elif minor != "0" and minor[0] == "0":
            yield line_num, f"Yocto-Version minor part must not have leading zeros: {version}"
...
def _yep_num(line_num: int, yep_number: str, prefix: str) -> MessageIterator:
    if yep_number == "":
        yield line_num, f"{prefix} must not be blank: {yep_number!r}"
        return
    if yep_number.startswith("0") and yep_number != "0":
        yield line_num, f"{prefix} must not contain leading zeros: {yep_number!r}"
    if not _is_digits(yep_number):
        yield line_num, f"{prefix} must be numeric: {yep_number!r}"
    elif not 0 <= int(yep_number) <= 9999:
        yield line_num, f"{prefix} must be between 0 and 9999: {yep_number!r}"
