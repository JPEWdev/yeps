...
class YEPBanner(rst.Directive):
    """Insert a special banner admonition in a YEP document."""
    ...
    def run(self) -> list[nodes.admonition]:
        ...
        admonition_node = self.admonition_class(
            "\n".join(source_lines), classes=["yep-banner"] + self.css_classes)
...
class CanonicalDocBanner(YEPBanner):
    """Insert an admonition pointing readers to a YEP's canonical docs."""

    admonition_pre_template = (
        "This YEP is a historical document. "
    ...
    admonition_post_text = (
        "See :yep:`1` for how to propose changes."
    )
    css_classes = ["canonical-doc", "sticky-banner"]


class CanonicalPyPASpecBanner(YEPBanner):
...
class CanonicalTypingSpecBanner(YEPBanner):
    """Insert a specialized admonition for the typing specification."""

    admonition_pre_template = (
        "This YEP is a historical document: see {link_content} for up-to-date "
...
class DeprecatedBanner(YEPBanner):
    """Generic admonition for deprecated YEPs."""

    admonition_class = nodes.warning
    admonition_pre_template = "{link_content}"
    admonition_pre_text = "This YEP has been deprecated."
    css_classes = ["sticky-banner", "deprecated"]


class RejectedBanner(DeprecatedBanner):
    """Insert an admonition for rejected YEPs."""

    admonition_pre_text = "This YEP has been rejected."
    css_classes = ["sticky-banner", "deprecated", "rejected"]


class SupersededBanner(YEPBanner):
    """Insert an admonition for superseded YEPs."""

    admonition_pre_template = "This YEP has been superseded by :yep:`{link_content}`."
    admonition_pre_text = "This YEP has been superseded."
    css_classes = ["sticky-banner", "deprecated", "superseded"]


class WithdrawnBanner(YEPBanner):
    """Insert an admonition for withdrawn YEPs."""

    admonition_pre_text = "This YEP has been withdrawn."
