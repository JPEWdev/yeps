...
class YEPContents(transforms.Transform):
    """Add TOC placeholder and horizontal rule after YEP title and headers."""
    ...
    def apply(self) -> None:
        ...
        # Insert the toc after title and YEP headers
...
class Contents(parts.Contents):
    ...
    def apply(self) -> None:
        contents = self.build_contents(self.document[0][4:])  # skip YEP title, headers, <hr/>, and contents
        if contents:
    ...
    def build_contents(self, node: nodes.Node | list[nodes.Node], _level: None = None):
        ...
        for section in children:
            ...
            # remove all pre-existing hyperlinks in the title (e.g. YEP references)
