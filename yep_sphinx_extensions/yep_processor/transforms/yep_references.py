...
class YEPReferenceRoleTitleText(transforms.Transform):
    ...
    def apply(self) -> None:
        ...
        for node in self.document.findall(nodes.reference):
            ...
            # get yep number and section target (fragment)
            yep_num, fragment = node.attributes.pop("_title_tuple")
            filename = f"yep-{yep_num:0>4}"
