...
class YEPFooter(transforms.Transform):
    """Footer transforms for YEPs.
    ...
    def apply(self) -> None:
        yep_source_path = Path(self.document["source"])
        if not yep_source_path.match("yep-????.???"):
        ...
        if yep_source_path.stem != "yep-0000":
            if yep_source_path.stem != "yep-0210":  # 210 is entirely empty, skip
                self.document += nodes.transition()
            self.document += _add_source_link(yep_source_path)
            self.document += _add_commit_history_info(yep_source_path)


def _add_source_link(yep_source_path: Path) -> nodes.paragraph:
    """Add link to source text on VCS (GitHub)"""
    source_link = f"https://github.com/JPEWdev/yeps/blob/main/yeps/{yep_source_path.name}"
    link_node = nodes.reference("", source_link, refuri=source_link)
    return nodes.paragraph("", "Source: ", link_node)


def _add_commit_history_info(yep_source_path: Path) -> nodes.paragraph:
    """Use local git history to find last modified date."""
    try:
        iso_time = _LAST_MODIFIED_TIMES[yep_source_path.stem]
    except KeyError:
        return nodes.paragraph()

    commit_link = f"https://github.com/JPEWdev/yeps/commits/main/yeps/{yep_source_path.name}"
...
def _get_last_modified_timestamps():
    ...
    # remove "yeps/" prefix from file names
    all_modified = all_modified.replace("\nyeps/", "\n")

    # set up the dictionary with the *current* files
    yeps_dir = Path(__file__, "..", "..", "..", "..", "yeps").resolve()
    last_modified = {path.stem: "" for path in yeps_dir.glob("yep-????.rst")}
    ...
    for change_set in change_sets:
        ...
        for file in files.strip().split("\n"):
            if not file.startswith("yep-") or not file.endswith((".rst", ".txt")):
                continue  # not a YEP
            file = file[:-4]
