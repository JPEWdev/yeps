...
from yep_sphinx_extensions.yep_processor.transforms import yep_zero
from yep_sphinx_extensions.yep_processor.transforms.yep_zero import _mask_email
from yep_sphinx_extensions.yep_zero_generator.constants import (
    SPECIAL_STATUSES,
    STATUS_ACCEPTED,
    STATUS_ACTIVE,
    STATUS_DEFERRED,
...
ABBREVIATED_STATUSES = {
    ...
    STATUS_SUPERSEDED: "Replaced by another succeeding YEP",
    STATUS_PROVISIONAL: "Provisionally accepted but additional feedback needed",
}

ABBREVIATED_TYPES = {
    TYPE_STANDARDS: "Normative YEP with a new feature for Python, implementation "
    "change for CPython or interoperability standard for the ecosystem",
    TYPE_INFO: "Non-normative YEP containing background, guidelines or other "
    "information relevant to the Python ecosystem",
    TYPE_PROCESS: "Normative YEP describing or proposing a change to a Python "
    "community process, workflow or governance",
}

class YEPParsingError(errors.SphinxError):
    pass


# YEPHeaders is identical to docutils.transforms.yeps.Headers excepting bdfl-delegate, sponsor & superseeded-by
class YEPHeaders(transforms.Transform):
    """Process fields in a YEP's initial RFC-2822 header."""

    # Run before yep_processor.transforms.yep_title.YEPTitle
    ...
    def apply(self) -> None:
        ...
        if not len(self.document):
            raise YEPParsingError("Document tree is empty.")

        header = self.document[0]
        if not isinstance(header, nodes.field_list) or "rfc2822" not in header["classes"]:
            raise YEPParsingError("Document does not begin with an RFC-2822 header; it is not a YEP.")

        # YEP number should be the first field
        yep_field = header[0]
        if yep_field[0].astext().lower() != "yep":
            raise YEPParsingError("Document does not contain an RFC-2822 'YEP' header!")

        # Extract YEP number
        yep_num_str = yep_field[1].astext()
        try:
            yep_num = int(yep_num_str)
        except ValueError:
            raise YEPParsingError(f"YEP header must contain an integer. '{yep_num_str}' is invalid!")

        # Special processing for YEP 0.
        if yep_num == 0:
            pending = nodes.pending(yep_zero.YEPZero)
            self.document.insert(1, pending)
            self.document.note_pending(pending)

        # If there are less than two headers in the preamble, or if Title is absent
        if len(header) < 2 or header[1][0].astext().lower() != "title":
            raise YEPParsingError("No title!")
        ...
        for field in header:
            ...
            if len(body) == 0:
                ...
                msg = f"YEP header field body contains multiple elements:\n{field.pformat(level=1)}"
                raise YEPParsingError(msg)
            elif not isinstance(body[0], nodes.paragraph):  # len(body) == 1
                msg = f"YEP header field body may only contain a single paragraph:\n{field.pformat(level=1)}"
                raise YEPParsingError(msg)

            para = body[0]
            if name in {"author", "bdfl-delegate", "yep-delegate", "sponsor"}:
                # mask emails
                for node in para:
                ...
                # replace YEP numbers with normalised list of links to YEPs
                new_body = []
                for yep_str in re.split(r",?\s+", body.astext()):
                    target = self.document.settings.yep_url.format(int(yep_str))
                    if self.document.settings.builder == "dirhtml":
                        target = f"../{target}"
                    new_body += [nodes.reference("", yep_str, refuri=target), nodes.Text(", ")]
...
def _process_pretty_url(url: str) -> tuple[str, str]:
    ...
    item_name = item_name.title().replace("Sig", "SIG").replace("Yep", "YEP")
    return item_name, item_type
...
def _abbreviate_status(status: str) -> str:
    ...
    try:
        ...
        raise YEPParsingError(f"Unknown status: {status}")
...
def _abbreviate_type(type_: str) -> str:
    try:
        ...
        raise YEPParsingError(f"Unknown type: {type_}")
